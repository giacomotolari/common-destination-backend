...
const flightsConditions = (fromRange: number, toRange: number, day: number, season: string) => {
    if (
        season === "May" ||
        season === "June" ||
        season === "July" ||
...
const purchasingSeason = (dailydate: any) => {
    let monthInNum = moment(dailydate).format("M");
...
const flightDurationString = (flightDuration: number) => {
...
const getFLightPrice = (percentage: number, flightDurationInHours: number) => {
    const normalPrice =
